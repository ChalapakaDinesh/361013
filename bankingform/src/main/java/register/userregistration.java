...
public class userregistration extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (username.isEmpty() || phoneNo.isEmpty() || address.isEmpty() || email.isEmpty() || password.isEmpty() || confirmPassword.isEmpty()) {
            ...
            pw.println("<script>alert('All fields must be filled.'); window.location.href='userregistration.html';</script>");
            return;
        }
        ...
        if (!password.equals(confirmPassword)) {
            PrintWriter pw = response.getWriter();
            pw.println("<script>alert('Password and Confirm Password do not match.'); window.location.href='userregistration.html';</script>");
            return;
        }
